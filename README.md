...
1. **新增 `clear` 强杀模式支持**：
   - 增加支持第4个命令行参数 `clear`。当执行带有此参数的命令时，不仅会执行原先的致盲回调清理操作，还会调用新增的 `RemoveAVForever()` 功能。
   - 实现包含强制结束防病毒进程、降权释放保护文件以及通过移动或任务管理器强制清理文件，来达到永久破坏杀软/EDR目的。
   - 扩充了大规模完整的**杀软/EDR进程黑名单**（`AVProcess[]` 数组涵盖了 360, 天擎, Windows Defender, 火绒, 卡巴斯基, 腾讯, 亚信, 安天, 趋势等关键进程）。
2. **完善驱动 3 (`wnBio.sys`) 和驱动 4 (`GPU-Z.sys`) 支持**：
   - 实现了针对 `wnBio.sys` 和 `GPU-Z.sys` 驱动完整的任意地址读写。特别是新增了通过物理地址到虚拟地址转换映射（`MmMapIoSpace` / `CR3` 缓存机制）的逻辑处理，增强利用的稳定性。
3. **修复线程对象类型指针解析错误**：
   - 将原先在寻找 `PsThreadType` 时使用的 `NtOpenThreadTokenEx` 改正为使用 `NtQueryInformationThread` 导出函数，并将特征搜索指令的深度上限从300扩大到600，保证了解析过程的准确度。
4. **优化 `ObRegisterCallbacks` 检测与清除输出**：
   - 重构了在解除进程与线程保护时的回调擦除逻辑，精准匹配到 `PreOperation` 和 `PostOperation` 函数指针，并在控制台更为清晰地输出对应驱动与其释放状态（以 `[clear]` 标识）。
